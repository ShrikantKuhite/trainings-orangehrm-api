...
public class DeleteJobDetails extends BaseTest {
	
	@Test
	public void deleteJobDetails() {		
		Response response = JobPortalAPI.deleteJobIdDetails(10);
		...
		// deleting the job 2nd time
		response = JobPortalAPI.deleteJobIdDetails(10);
		response.prettyPrint();
		Assert.assertEquals(response.getStatusCode(), 404,"Status code is incorrect");
		Reporter.log("Status Code is verified successfully as : "+response.getStatusCode());
		ExtentListeners.extentTest.log(Status.PASS, "Status Code is verified successfully as : "+response.getStatusCode());
	}
